...
If you do **not** see a **Report a vulnerability** button, Private Vulnerability Reporting may not be enabled yet — please use the fallback email channel below.

<!-- Maintainer: PVR must be enabled for the steps above to work. Enable at
     Settings → Advanced Security → Private vulnerability reporting → Enable,
     or: gh api --method PUT repos/AojdevStudio/development-kit/private-vulnerability-reporting -->
...
- **Affected version / commit** — the tag (e.g. `v0.2.0`) or the `main` commit SHA you tested against.
...
This is a pre-1.0 project. It is consumed by **forking the repository** and building product features into the spine; there is no distributed released binary or published package.

| Version / branch | Supported |
|---|---|
| `main` (latest commit) | Yes — fixes land here |
| Latest tagged line (currently `v0.2.0`, source-only) | Yes — best-effort, most recent line only |
| Older `0.x` snapshots / tags | No backports |
| Forks / products built from the kit | No — see below |

Notes:

- `v0.2.0` is the latest line (referenced in `CHANGELOG`); there are no older release tags to back-port to.
- SemVer is declared, but the project is in `0.x`: API and authority surfaces can still change between minor versions.
- Security fixes land on `main`. There is **no coordinated multi-version patch release and no back-porting** to older `0.x` snapshots.
...
*This policy applies to the `main` branch and the latest tagged line of `AojdevStudio/development-kit`, and lives at one of GitHub's recognized paths (`SECURITY.md` in the repo root, `/docs`, or `/.github`) so it is auto-linked from the Security tab. It may change as the project matures toward 1.0.*
